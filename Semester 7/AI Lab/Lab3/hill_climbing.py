...
from hill_climbing_util import h_n, find_neighbours
...
def hill_climbing(initial_matrix: Matrix, goal: Matrix, heuristic_used: int):
    ...
    while Explored_But_Not_Visited.qsize() > 0:
    	print('----------')
    	current_matrix = Explored_But_Not_Visited.get()
    	#print(current_matrix.state.mat)
    	...
    	for neighbour in neighbours:
    		...
    		if neighbour_hash not in closed_list:
    			...
    			neighbour_state = Curr_State_Info(neighbour, current_matrix.g_n + 1, h_n(neighbour, goal, heuristic_used))
    			neighbour_queue.put(neighbour_state)
    		elif current_matrix.g_n + 1 < closed_list[neighbour_hash]:
    			parent_list[neighbour_hash] = Matrix_hash
    			
    			neighbour_state = Curr_State_Info(neighbour, current_matrix.g_n + 1, h_n(neighbour, goal, heuristic_used))
    			neighbour_queue.put(neighnour_state)
    		if neighbour_queue.empty():
    			break
    		best_neighbour = neighbour_queue.get()
    		if best_neighbour <= current_matrix:
    			Explored_But_Not_Visited.put(best_neighbour)
    			best_neighbour_hash = ''.join(str(val) for row in best_neighbour.state.mat for val in row)
    			closed_list[best_neighbour_hash] = current_matrix.g_n + 1
